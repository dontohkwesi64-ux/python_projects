...
def analyze_students():
    if not students:
        messagebox.showinfo("Analysis", "No student records available.")
        return
    ...
    for s in students:
        ...
        if grade in ["D", "E", "F"]:
            low_performers.append(s['name'])
    
    # Average age
    avg_age = sum([s['age'] for s in students])/total
    
    # Prepare analysis text
    analysis_text = f"Total Students: {total}\nAverage Age: {avg_age:.1f}\n\n"
    analysis_text += "Grade Distribution:\n"
    for g in ["A","B","C","D","E","F"]:
        analysis_text += f"{g}: {grade_counts[g]}\n"
    ...
    analysis_window.title("AI Analysis – Bossu Dashboard")
    analysis_window.geometry("400x400")
    analysis_window.configure(bg="#f0f4f7")
    
    tk.Label(analysis_window, text="AI Analysis", font=("Segoe UI",14,"bold"), bg="#2b5797", fg="white").pack(fill="x", pady=5)
    
    text_box = tk.Text(analysis_window, wrap="word", height=15, width=45)
    text_box.pack(padx=10, pady=10)
    text_box.insert("1.0", analysis_text)
    text_box.config(state="disabled")
    
    # Pie chart button
    tk.Button(analysis_window, text="Show Pie Chart", bg="#2b5797", fg="white", command=lambda: show_pie_chart(grade_counts), width=20).pack(pady=10)

def show_pie_chart(grade_counts):
    labels = []
    sizes = []
...
# --- Tkinter GUI ---
root = tk.Tk()
root.title("Bossu Dashboard Edition – 5.3")
...
tk.Button(frame_search, text="Search", bg="#2b5797", fg="white", command=search_student).grid(row=0, column=2, padx=10)
...
tk.Button(frame_buttons, text="AI Analysis", command=analyze_students, bg="#5cb85c", fg="white", width=15).grid(row=0, column=4, padx=5)
