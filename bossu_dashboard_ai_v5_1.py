...
def show_grade_chart():
    ...
    grades = [s['age'] for s in students]  # or use percentages if available
    colors = []
    for s in students:
        if s['grade'] in ["A","B"]:
            colors.append("green")
...
# --- Tkinter GUI ---
root = tk.Tk()
root.title("Bossu Dashboard AI – 5.2")
root.geometry("780x550")
root.configure(bg="#f0f4f7")

# Header
tk.Label(root, text="Bossu AI Student Dashboard", font=("Segoe UI", 16, "bold"),
...
tk.Button(frame_buttons, text="Show Grade Chart", command=show_grade_chart, width=18, bg="#ffcc00").grid(row=0, column=4, padx=5)
...
def show_grade_chart():
    ...
    names = [s['name'] for s in students]
    grades = [ord(s['grade']) for s in students]  # A=65, B=66... F=70

    plt.figure(figsize=(8,5))
    plt.bar(names, grades, color='skyblue')
    plt.xlabel("Students")
    plt.ylabel("Grades (A=65 ... F=70)")
    plt.title("Student Grades Overview")
    plt.show()
tk.Button(frame_buttons, text="Show Grades Chart", command=show_grade_chart, bg="#ffc107").grid(row=0, column=4, padx=5)
